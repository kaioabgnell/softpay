...
@section('page-title', 'Novo Template')
    ...
    <div style="background:#ffffff; border-radius:12px; padding:32px; box-shadow:0 1px 2px rgba(0,0,0,.05); border:1px solid #e5e7eb;">
        <form method="POST" action="{{ route('templates.store') }}">
            ...
            <div style="margin-bottom:20px;">
                ...
                <input type="text" id="nome" name="nome" value="{{ old('nome') }}" required maxlength="72"
            ...
            <div style="margin-bottom:20px;">
                <label style="display:block; font-size:13px; font-weight:500; color:#6b7280; margin-bottom:6px;" for="meta_id">
                    Nome do template na Meta <span style="font-weight:400;">(exatamente como aparece em "Name" no Kapso, ex: promo_template — não é o ID numérico)</span>
                </label>
                <input type="text" id="meta_id" name="meta_id" value="{{ old('meta_id') }}" required maxlength="64" placeholder="template1"
            ...
            <div style="margin-bottom:20px;">
                <label style="display:block; font-size:13px; font-weight:500; color:#6b7280; margin-bottom:6px;" for="idioma">
                    Idioma <span style="font-weight:400;">(código do template na Meta)</span>
                </label>
                <select id="idioma" name="idioma" required
                        ...
                        onfocus="this.style.borderColor='#111111'" onblur="this.style.borderColor='#e5e7eb'">
                    @php $idioma = old('idioma', 'pt_BR'); @endphp
            ...
            <div style="margin-bottom:20px;">
                <label style="display:block; font-size:13px; font-weight:500; color:#6b7280; margin-bottom:6px;" for="categoria">Categoria <span style="font-weight:400;">(opcional)</span></label>
                <select id="categoria" name="categoria"
                        style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:8px; font-size:14px; font-family:inherit; outline:none; color:#111111; background:#fff;"
                        onfocus="this.style.borderColor='#111111'" onblur="this.style.borderColor='#e5e7eb'">
                    <option value="">— Selecione —</option>
                    <option value="marketing" {{ old('categoria') === 'marketing' ? 'selected' : '' }}>Marketing</option>
                    <option value="utility" {{ old('categoria') === 'utility' ? 'selected' : '' }}>Utility</option>
                    <option value="authentication" {{ old('categoria') === 'authentication' ? 'selected' : '' }}>Authentication</option>
                </select>
            </div>

            <div style="margin-bottom:28px; display:flex; align-items:center; gap:10px;">
                <input type="hidden" name="usa_client_name" value="0">
                <input type="checkbox" id="usa_client_name" name="usa_client_name" value="1" {{ old('usa_client_name') ? 'checked' : '' }}
